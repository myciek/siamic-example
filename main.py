...
def get_batch(batch_size=64):
    ...
    batch_x.append(np.zeros((batch_size, 10, 2)))
    batch_x.append(np.zeros((batch_size, 10, 2)))
...
def nway_one_shot(model, n_way, n_val):
    ...
    for i in class_list:
        ...
        temp = [np.zeros((n_way, 10, 2)), np.zeros((n_way, 10, 2))]
    ...
    with open(os.path.join(light_dir, file_name)) as light_file:
        with open(os.path.join(sound_dir, file_name)) as sound_file:
            ...
            for row in light_reader:
                ...
                temp_x.append(np.asarray([row[0], sound]))
                counter += 1
                if counter == 10:
                    temp.append(len(x))
                    x.append(temp_x)
...
# Training Split
train_files = train_size * no_of_items_in_each_class
